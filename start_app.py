...
if __name__ == '__main__':
    ...
    KMfile = ["kmeans_data_original.txt",
        "kmeans_data_ten_head_random.txt",
        "kmeans_data_ten_tail_random.txt",
        "kmeans_data_ten_random_random.txt"]

    for i, val in enumerate(excuate_list):
        job: str = ("bin/spark-submit " \
            ...
            "--executor-memory 6g " \
            "--name {name} " \
            "/home/spark/Downloads/KMeans/target/scala-2.11/KMeans.jar " \
            "hdfs://192.168.122.184:9000/KMeans_Data/{file} " \
            "10 20 10 1").format(name = KMmethod[val], file = KMfile[val])
        main(job, folder_name = KMmethod[val])

    LRmethod = ["LR_O",
                "LR_H",
                "LR_T",
                "LR_R"]
    LRfile = ["LR_data_original.csv",
        "LR_data_head_random.csv",
        "LR_data_tail_random.csv",
        "LR_data_random_random.csv"]
    ...
    for i, val in enumerate(excuate_list):
        job: str = ("bin/spark-submit " \
                ...
                "hdfs://192.168.122.51:9000/LR_data/{file} " \
                "10 10 10 10 " \
                "100 ").format(name = LRmethod[val], file = LRfile[val])
        main(job, folder_name = LRmethod[val])
